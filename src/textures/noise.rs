...
pub fn noise(x: Float, y: Float, z: Float) -> Float {
    ...
    // converting very large texture coordinates to cell coordinates.
...
pub fn d_noise(p: &Point3f) -> Vector3f {
    let delta = 0.01;
    let n = noise(p.x, p.y, p.z);
    let nx = noise(p.x + delta, p.y, p.z);
    let ny = noise(p.x, p.y + delta, p.z);
    let nz = noise(p.x, p.y, p.z + delta);
    Vector3f::new(nx - n, ny - n, nz - n) / delta
}
