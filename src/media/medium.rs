// Re-export from base::medium for backward compatibility.
pub use crate::base::medium::Medium;

use super::cloud::CloudMedium;
use super::grid::GridMedium;
use super::homogeneous::HomogeneousMedium;
use super::nanovdb::NanoVDBMedium;
use super::rgb_grid::RGBGridMedium;
use crate::paramdict::*;
use crate::util::base::*;
...
fn scale_spectrum(source: Spectrum, scale: Float) -> Spectrum {
    if scale == 1.0 {
        return source;
    }
    source * scale
...
pub fn medium_scattering_parameters(
    parameters: &ParameterDictionary,
) -> (Spectrum, Spectrum, Float, Float) {
    ...
    let (sig_a_default, sig_s_default) =
        if let Some((a, b)) = get_medium_scattering_properties(&preset) {
            (
                parameters.get_one_spectrum_typed("sigma_a", &a, SpectrumType::Unbounded),
                parameters.get_one_spectrum_typed("sigma_s", &b, SpectrumType::Unbounded),
            )
        } else {
            if !preset.is_empty() {
                warn!("Medium preset \"{}\" not found. Using defaults.", preset);
            }
    ...
    (
        scale_spectrum(sig_a_source, scale),
        scale_spectrum(sig_s_source, scale),
        scale,
        g,
    )
}

pub fn grid_resolution(parameters: &ParameterDictionary) -> (u32, u32, u32) {
    (
        parameters.get_one_int("nx", 1) as u32,
        parameters.get_one_int("ny", 1) as u32,
        parameters.get_one_int("nz", 1) as u32,
    )
}

pub fn load_ascii_density_grid(path: &str, expected: usize) -> Option<Vec<Float>> {
    if path.is_empty() || expected == 0 {
        return None;
    }
    let text = fs::read_to_string(path).ok()?;
    let values: Vec<Float> = text
        .split_whitespace()
        .filter_map(|s| s.parse::<Float>().ok())
        .collect();
...
impl Medium {
    ...
    ) -> Result<Medium, PbrtError> {
        match name {
            "homogeneous" => HomogeneousMedium::create(parameters).map(Into::into),
            "uniformgrid" => GridMedium::create(parameters, render_from_medium).map(Into::into),
            "rgbgrid" => RGBGridMedium::create(parameters, render_from_medium).map(Into::into),
            "cloud" => CloudMedium::create(parameters, render_from_medium).map(Into::into),
            "nanovdb" => NanoVDBMedium::create(parameters, render_from_medium).map(Into::into),
