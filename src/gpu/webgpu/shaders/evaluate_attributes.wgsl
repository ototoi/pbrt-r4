...
fn evaluate_attributes(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (surface.material_tree_layout >= arrayLength(&material_tree_layouts)) { set_render_error(); return; }
    let tree_layout = material_tree_layouts[surface.material_tree_layout];
    if (tree_layout.node_count == 0u || tree_layout.node_count > material_table.attributes_eval_stride || tree_layout.node_offset + tree_layout.node_count > arrayLength(&material_tree_nodes)) { set_render_error(); return; }
    let base = queue_index * material_table.attributes_eval_stride; let lambda = load_sample_lambda(pixel); let samples = load_ray_samples(pixel); var node_index = tree_layout.node_offset;
    for (var visited = 0u; visited < tree_layout.node_count; visited++) {
        let node = material_tree_nodes[node_index]; var e = evaluate_material_attributes(node_index, lambda);
        ...
        if (node.child0 != 0xffffffffu) { e.child_work_item0 = base + node.child0 - tree_layout.node_offset; }
        if (node.child1 != 0xffffffffu) { e.child_work_item1 = base + node.child1 - tree_layout.node_offset; }
        ...
        attributes_eval_work_items[base + node_index - tree_layout.node_offset] = e; node_index = next_material_tree_node(tree_layout, node_index);
    }
}
